export default ({ userId }) => {
  const state = {
    sectionInfo: {
      id: `${userId}-giveaway-1-french`,
      translateText: "Traduit de l'anglais",
      defaultCommentsToShow: 5,
      defaultCommentsToLoadAtOnce: 10,
      defaultRepliesToShow: 5,
      defaultRepliesToLoadAtOnce: 10,
      generalProductName: '',
    },
    user: {
      id: 134,
      name: 'Customer Comment',
      url: '',
      image: '/images/noprofilepic.jpg',
      affiliation: {
        name: '',
        url: '',
      },
    },
    sortBy: {
      top: true,
      newest: false,
      oldest: false,
    },
    list: {
      give3frepo001b: {
        id: 'give3frepo001b',
        content: "BAM PRODUCT_NAME vient d'apparaître dans mes news... j'ai pensé, eh pourquoi pas ? j'ai juste cliqué sur le lien, je l'ai mis dans mes infos et ensuite... BOUM SI « TU GAGNES ! »<3<3<3<3",
        user: {
          id: 60,
          name: 'Taryn Mackenzie',
          url: 'https://www.facebook.com/profile.php?id=100010880380081',
          image: 'https://scontent-sjc2-1.xx.fbcdn.net/v/t1.0-1/p160x160/10342780_115308208841858_771440735056581410_n.jpg?oh=5199fc88c6e5cefb414da0dbcd43b681&oe=5A38DD8E',
          affiliation: {
            name: 'Seattle, Washington',
            url: 'https://www.facebook.com/places/Things-to-do-in-Seattle-Washington/110843418940484/',
          },
        },
        options: {
          collapse: {
            enabled: true,
            content: 'Collapse comment',
          },
          spam: {
            enabled: true,
            content: 'Mark as spam',
          },
          report: {
            enabled: true,
            content: 'Report',
          },
          edit: {
            enabled: false,
            content: 'Edit comment',
          },
          delete: {
            enabled: false,
            content: 'Delete comment',
          },
        },
        likes: 7,
        relativeDate: [0, 22, 24],
        report: false,
        spam: false,
        showing: true,
        editing: false,
        liked: false,
        edited: false,
      },
      give3frenu003a: {
        id: 'give3frenu003a',
        content: "J'ai gagné quelques petits prix d'un concours publicitaire d'une compagnie de tabac et des gains instantanés. Rien de trop passionnant ces derniers temps, mais cela nourrit l'espoir d'un gros gain un jour !",
        user: {
          id: 61,
          name: 'Sheila Halloway',
          url: 'https://www.facebook.com/profile.php?id=100011886491967',
          image: 'https://scontent-sjc2-1.xx.fbcdn.net/v/t1.0-1/p160x160/13077055_108820012857548_122161349102974062_n.jpg?oh=02f0bbbc3a7b792728fde98f41ba022a&oe=5A2F90DF',
          affiliation: {
            name: 'Los Angeles, California',
            url: 'https://www.facebook.com/places/Things-to-do-in-Los-Angeles-California/110970792260960/',
          },
        },
        options: {
          collapse: {
            enabled: true,
            content: 'Collapse comment',
          },
          spam: {
            enabled: true,
            content: 'Mark as spam',
          },
          report: {
            enabled: true,
            content: 'Report',
          },
          edit: {
            enabled: false,
            content: 'Edit comment',
          },
          delete: {
            enabled: false,
            content: 'Delete comment',
          },
        },
        likes: 3,
        relativeDate: [2, 2, 27],
        report: false,
        spam: false,
        showing: true,
        editing: false,
        liked: false,
        edited: false,
      },
      give3frepo003a: {
        id: 'give3frepo003a',
        content: "« J'ai eu un billet de loto pendant un certain temps et je ne pensais pas que la vie puisse s'améliorer d'une quelconque façon ; c'était mon rêve de futur, ma part de chance en charpie au fond de ma poche. Puis j'ai fait tomber mon e-mail dans cette fontaine à profits. Ma vie s'en est améliorée à 500 %, je ne mens pas. »",
        user: {
          id: 62,
          name: 'Bobbie Marsh',
          url: 'https://www.facebook.com/profile.php?id=100011953211991',
          image: 'https://scontent-lax3-1.xx.fbcdn.net/v/t1.0-1/p160x160/13082731_106892633052510_4388890863487426587_n.jpg?oh=2469d271dc6a26cb8858da5a2de86790&oe=5A656EE2',
          affiliation: {
            name: '',
            url: '',
          },
        },
        options: {
          collapse: {
            enabled: true,
            content: 'Collapse comment',
          },
          spam: {
            enabled: true,
            content: 'Mark as spam',
          },
          report: {
            enabled: true,
            content: 'Report',
          },
          edit: {
            enabled: false,
            content: 'Edit comment',
          },
          delete: {
            enabled: false,
            content: 'Delete comment',
          },
        },
        likes: 12,
        relativeDate: [4, 21, 14],
        report: false,
        spam: false,
        showing: true,
        editing: false,
        liked: false,
        edited: false,
      },
      give3frenu004a: {
        id: 'give3frenu004a',
        content: "Cela fait un peu plus d'un mois que je participe pour gagner toutes sortes de cadeaux que je vois passer et je n'ai toujours rien gagné. Je sais que cela peut prendre du temps, mais cela me fait me demander si je ne ferais pas fausse route.. Habituellement je ne vais pas au bout de l'inscription ou je visite juste le site Internet/regarde le film... Parce que habituellement je ne m'abonne à rien/ne suis rien puisque mes médias sociaux personnels sont liés dans les serveurs Gleam, et peut-être est-il exigé que toutes les participations soient complétées sur la plupart d'entre eux pour avoir une chance de gagner et je ne sais rien à ce sujet.",
        user: {
          id: 63,
          name: 'Ashley Houston',
          url: 'https://www.facebook.com/profile.php?id=100010731075519',
          image: 'https://scontent-sjc2-1.xx.fbcdn.net/v/t1.0-1/p160x160/13010846_224697677897933_3685544167206442586_n.jpg?oh=013043aa20c6131d04b8dd2d3d6baa26&oe=5A3575A5',
          affiliation: {
            name: 'San Diego, California',
            url: 'https://www.facebook.com/places/Things-to-do-in-San-Diego-California/110714572282163/',
          },
        },
        options: {
          collapse: {
            enabled: true,
            content: 'Collapse comment',
          },
          spam: {
            enabled: true,
            content: 'Mark as spam',
          },
          report: {
            enabled: true,
            content: 'Report',
          },
          edit: {
            enabled: false,
            content: 'Edit comment',
          },
          delete: {
            enabled: false,
            content: 'Delete comment',
          },
        },
        likes: 12,
        relativeDate: [5, 1, 40],
        report: false,
        spam: false,
        showing: true,
        editing: false,
        liked: false,
        edited: false,
        replies: {
          give3frenu004ar1: {
            id: 'give3frenu004ar1',
            parentId: 'give3frenu004a',
            content: "Habituellement, la majeure partie des participations s'y trouvent, afin d'avoir les meilleures chances vous avez certainement intérêt à vous occuper des médias sociaux.",
            user: {
              id: 61,
              name: 'Sheila Halloway',
              url: 'https://www.facebook.com/profile.php?id=100011886491967',
              image: 'https://scontent-sjc2-1.xx.fbcdn.net/v/t1.0-1/p160x160/13077055_108820012857548_122161349102974062_n.jpg?oh=02f0bbbc3a7b792728fde98f41ba022a&oe=5A2F90DF',
              affiliation: {
                name: 'Los Angeles, California',
                url: 'https://www.facebook.com/places/Things-to-do-in-Los-Angeles-California/110970792260960/',
              },
            },
            options: {
              collapse: {
                enabled: false,
                content: 'Collapse comment',
              },
              spam: {
                enabled: true,
                content: 'Mark as spam',
              },
              report: {
                enabled: true,
                content: 'Report',
              },
              edit: {
                enabled: false,
                content: 'Edit comment',
              },
              delete: {
                enabled: false,
                content: 'Delete comment',
              },
            },
            liked: false,
            likes: 2,
            relativeDate: [2, 2, 31],
            report: false,
            spam: false,
            edited: false,
          },
        },
      },
      give3frenu006a: {
        id: 'give3frenu006a',
        content: "Voici la meilleure astuce qui m'a été donnée : il suffit d'utiliser le remplissage automatique de Google Chrome pour tous les formulaires et cela semble assez bien faire le job.",
        user: {
          id: 64,
          name: 'Alvin Benedict',
          url: 'https://www.facebook.com/alvin.benedict.77',
          image: 'https://scontent-sjc2-1.xx.fbcdn.net/v/t1.0-1/p160x160/14225412_115345802256165_107315515155352405_n.jpg?oh=7c048b4183108f92533639df14a43bd1&oe=5A390249',
          affiliation: {
            name: 'Arnaudville, Louisiana',
            url: 'https://www.facebook.com/places/Things-to-do-in-Arnaudville-Louisiana/108140032539631/',
          },
        },
        options: {
          collapse: {
            enabled: true,
            content: 'Collapse comment',
          },
          spam: {
            enabled: true,
            content: 'Mark as spam',
          },
          report: {
            enabled: true,
            content: 'Report',
          },
          edit: {
            enabled: false,
            content: 'Edit comment',
          },
          delete: {
            enabled: false,
            content: 'Delete comment',
          },
        },
        likes: 76,
        relativeDate: [7, 3, 28],
        report: false,
        spam: false,
        showing: true,
        editing: false,
        liked: false,
        edited: false,
      },
      give3frepo005a: {
        id: 'give3frepo005a',
        content: "J'ai gagné mon premier cadeau sur PRODUCT_NAME ! Il s'agit d'une carte-cadeau de 500 $ (bien que je ne soit pas sûr(e) à 100 % que ce soit une Visa ou une carte de magasin) d'une entreprise de café. J'ai juste commencé à participer frénétiquement à des concours il y a 2 semaines. Eeeeeeet maintenant, je suis accro.",
        user: {
          id: 66,
          name: 'Carla Ramos',
          url: 'https://www.facebook.com/profile.php?id=100012182859842',
          image: 'https://scontent-sjc2-1.xx.fbcdn.net/v/t1.0-1/p160x160/13239072_114916385591123_4171036202608140005_n.jpg?oh=c038bc9e7c46d582d7aefc616422cfc4&oe=5A25DCD8',
          affiliation: {
            name: 'Ohio Christian University',
            url: 'https://www.facebook.com/OhioChristian/?pnref=lhc',
          },
        },
        options: {
          collapse: {
            enabled: true,
            content: 'Collapse comment',
          },
          spam: {
            enabled: true,
            content: 'Mark as spam',
          },
          report: {
            enabled: true,
            content: 'Report',
          },
          edit: {
            enabled: false,
            content: 'Edit comment',
          },
          delete: {
            enabled: false,
            content: 'Delete comment',
          },
        },
        likes: 84,
        relativeDate: [8, 12, 16],
        report: false,
        spam: false,
        showing: true,
        editing: false,
        liked: false,
        edited: false,
      },
    },
  };
  return state;
};
